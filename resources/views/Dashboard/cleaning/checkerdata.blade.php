  ...
  <div class="overflow-x-auto bg-white rounded-xl shadow border border-gray-300 my-4">
    <table class="min-w-full table-auto text-sm text-gray-800 border-collapse">
      <thead class="bg-teal-1001 text-white sticky top-0">
        <tr>
          <th class="border border-gray-300 px-4 py-3 text-left">{{ __('dashboardCleaning.checkdata.table.date') }}</th>
          <th class="border border-gray-300 px-4 py-3 text-left">{{ __('dashboardCleaning.checkdata.table.name') }}</th>
          <th class="border border-gray-300 px-3 py-3 text-center">{{ __('dashboardCleaning.checkdata.table.room') }}</th>
          ...
          <th class="border border-gray-300 px-3 py-3 text-center">{{ __('dashboardCleaning.checkdata.table.total') }}</th>
        </tr>
      </thead>
      <tbody class="text-sm">
        @forelse ($checkerData as $data)
          <tr class="border-t border-gray-200 hover:bg-yellow-50">
            ...
            <td class="text-center">{{ $data['pembersihan_khusus'] ? '✅ + ' . ($data['poin']->pembersihan_khusus ?? '-') : '❌' }}</td>
            <td class="text-center">{{ $data['mengangkat_barang'] ? '✅ + ' . ($data['poin']->mengangkat_barang ?? '-') : '❌' }}</td>
            <td class="text-center">{{ $data['membersihkan_gudang'] ? '✅ + ' . ($data['poin']->membersihkan_gudang ?? '-') : '❌' }}</td>
            <td class="text-center">{{ $data['obat_pool'] ? '✅ + ' . ($data['poin']->obat_pool ?? '-') : '❌' }}</td>
            <td class="text-center">{{ $data['membersihkan_pool'] ? '✅ + ' . ($data['poin']->membersihkan_pool ?? '-') : '❌' }}</td>
        ...
        @empty
          <tr>
            <td colspan="12" class="text-center text-gray-500 py-6">{{ __('dashboardCleaning.checkdata.table.no_data') }}</td>
          </tr>
        @endforelse
      </tbody>
    </table>
