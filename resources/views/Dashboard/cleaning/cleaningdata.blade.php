...
<div class="w-full max-w-screen-2xl mx-auto h-[calc(100vh-80px)] overflow-y-auto px-4 py-6">
  ...
  @foreach($grouped as $groupKey => $cleanings)
    ...
    <div class="mb-10">
      <h2 class="text-2xl font-semibold mb-4 text-blue-800">{{ $buildingName }} — <span class="text-gray-600">{{ $memberCount }} {{ __('dashboardCleaning.cleaningdata.members') }}</span></h2>
                      ...
                      <tr class="{{ $loop->parent->iteration % 2 === 0 ? 'bg-slate-100' : 'bg-white' }} hover:bg-yellow-50">
                        ...
                        @if($index === 0)
                          ...
                          <td class="px-6 py-4 bg-blue-100" rowspan="{{ $cleaning['member_count'] }}">
                            <div>{{ __('dashboardCleaning.cleaningdata.table.total_info') }}: {{ number_format($cleaning['total'], 1) }}</div>
                            <div class="text-gray-500">/ {{ $cleaning['member_count'] }} {{ __('dashboardCleaning.cleaningdata.table.member') }}</div>
                            <div class="mt-1 font-bold text-base text-blue-800">= {{ number_format($cleaning['poin_per_member'], 2) }}</div>
                          </td>
                        @endif
