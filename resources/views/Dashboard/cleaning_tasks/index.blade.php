...
@section('content')
  <div class="w-full max-w-screen-2xl mx-auto h-[calc(100vh-80px)] overflow-y-auto px-4 py-6">
    ...
    <div class="flex justify-end gap-4">
      <a href="{{ route('cleaningTasks.create') }}"
        ...
        <tbody>
          @forelse ($tasks as $task)
            <tr class="bg-white even:bg-teal-50 border-b border-gray-200 hover:bg-yellow-50 text-center">
              ...
              <td class="px-6 py-4">
                <span
                  class="px-2 py-1 rounded text-xs font-semibold {{ $task->status == 'active' ? 'bg-green-200 text-green-800' : 'bg-red-200 text-red-800' }}">
                  {{ ucfirst($task->status) }}
                </span>
              </td>
              <td class="px-6 py-4 flex flex-wrap gap-2 justify-center">
                <a href="{{ route('cleaningTasks.edit', $task->id) }}"
