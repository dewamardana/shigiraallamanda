...
<div class="w-full max-w-screen-2xl mx-auto h-[calc(100vh-80px)] overflow-y-auto px-4 py-6">
  ...
  <a href="{{  route('formulaCheck.create') }}" class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2">Tambah Formula</a>
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg mt-4">
        <table class="w-full text-sm text-left rtl:text-right text-gray-500">
            <thead class="text-xs text-white bg-teal-1001 uppercase text-center">
                <tr>
                    <th scope="col" class="px-6 py-3">Nama Formula</th>
            ...
            <tbody>
              @foreach ($formulas as $formula)
                <tr class="bg-white even:bg-teal-50 border-b border-gray-200 hover:bg-yellow-50 text-center">
                    ...
                    <td class="px-6 py-4">
                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $formula->active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                            {{ $formula->active ? 'Aktif' : 'Nonaktif' }}
                        </span>
                    </td>
                    <td class="px-6 py-4">
                        <a href="{{ route('formulaCheck.show', $formula->id) }}" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2 me-2 mb-2 focus:outline-none">Show</a>
                        <a href="{{ route('formulaCheck.edit', $formula->id) }}" class="focus:outline-none text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2 me-2 mb-2">Edit</a>
                        <form action="{{ route('formulaCheck.destroy', $formula->id) }}" method="POST" class="inline-block" data-confirm="{{ __('dashboardFormulaIndex.table.delete_confirm') }}">
                            @csrf
                            @method('DELETE')
