...
@elseif (session('error'))
  ...
  <h1 class="text-3xl font-bold mb-8 text-gray-800">{{ __('dashboardFormulaCheck.index.title') }}</h1>
  <a href="{{ route('formulaCheck.create') }}"
  ...
  <div class="relative overflow-x-auto shadow-md sm:rounded-lg mt-4">
    <table class="w-full text-sm text-left rtl:text-right text-gray-500">
      <thead class="text-xs text-white bg-teal-1001 uppercase text-center">
        <tr>
          <th scope="col" class="px-6 py-3">{{ __('dashboardFormulaCheck.index.table.name') }}</th>
          <th scope="col" class="px-6 py-3">{{ __('dashboardFormulaCheck.index.table.description') }}</th>
          <th scope="col" class="px-6 py-3">{{ __('dashboardFormulaCheck.index.table.status') }}</th>
          <th scope="col" class="px-6 py-3">{{ __('dashboardFormulaCheck.index.table.action') }}</th>
        </tr>
      </thead>
      <tbody>
        @foreach ($formulas as $formula)
          <tr class="bg-white even:bg-teal-50 border-b border-gray-200 hover:bg-yellow-50 text-center">
            ...
            <td class="px-6 py-4">
              <span
                class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $formula->active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                {{ $formula->active ? __('dashboardFormulaCheck.index.table.active') : __('dashboardFormulaCheck.index.table.inactive') }}
              </span>
            </td>
            <td class="px-6 py-4">
              <a href="{{ route('formulaCheck.show', $formula->id) }}"
              ...
              <a href="{{ route('formulaCheck.edit', $formula->id) }}"
              ...
              <form action="{{ route('formulaCheck.destroy', $formula->id) }}" method="POST" class="inline-block"
                data-confirm="{{ __('dashboardFormulaCheck.index.table.delete_confirm') }}">
                @csrf
                @method('DELETE')
                <button type="submit"
