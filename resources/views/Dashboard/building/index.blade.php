  ...
  <div class="relative overflow-x-auto shadow-md sm:rounded-lg mt-4">
    <table class="w-full text-sm text-left rtl:text-right text-gray-500">
      <thead class="text-xs text-white bg-teal-1001 uppercase text-center">
        <tr>
          <th scope="col" class="px-6 py-3">{{ __('dashboardBuilding.index.table.photo') }}</th>
          <th scope="col" class="px-6 py-3">{{ __('dashboardBuilding.index.table.name') }}</th>
          <th scope="col" class="px-6 py-3">{{ __('dashboardBuilding.index.table.description') }}</th>
      ...
      <tbody>
        @foreach ($building as $build)
          <tr class="bg-white even:bg-teal-50 border-b border-gray-200 hover:bg-yellow-50 text-center">
            <td class="px-6 py-4">
              <img src="{{ $build->foto ? asset('storage/' . $build->foto) : 'https://via.placeholder.com/150' }}"
