...
<div class="bg-white p-5 rounded-xl shadow-2xl m-5 mx-auto max-w-5xl w-full">
  <h2 class="font-bold text-4xl text-center text-black-500 mb-8">{{ __('dashboardBuildingEdit.title') }}</h2>

  <form action="{{ route('building.update',['building' => $building->slug]) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <div class="flex flex-col md:flex-row gap-8 justify-center p-6">
      ...
      <div class="w-full md:max-w-xs">
        <h6 class="mb-2 text-lg font-semibold text-center">{{ __('dashboardBuildingEdit.building_image') }}</h6>
        <p class="text-xs text-slate-500 mb-6 text-center">{{ __('dashboardBuildingEdit.image_hint') }}</p>
        <div class="relative w-40 h-40 rounded-xl overflow-hidden mb-6 border-2 border-indigo-600 group cursor-pointer block mx-auto">
          <img id="preview-foto" src="{{ $building->foto ? asset('storage/'.$building->foto) : 'https://via.placeholder.com/150' }}" class="object-cover w-full h-full">
      ...
      <div class="w-full space-y-5">
        ...
        <div>
            ...
            <input type="text" name="building_name" id="building_name" value="{{ old('building_name', $building->building_name) }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required />                  
          @error('building_name')
          <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
          @enderror
        </div>
        ...
        <div>
            <label for="description" class="block mb-2 text-sm font-medium text-teal-1001">{{ __('dashboardBuildingEdit.description') }}</label>
            <textarea name="description" id="description" value="{{ old('description', $building->description) }}" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500">{{ $building->description }}</textarea>  
          @error('description')
          <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
          @enderror
        </div>
        <div class="flex justify-center gap-4 mt-6">
            <button type="submit"
                ...
                {{ __('dashboardBuildingEdit.button') }}
            </button>
