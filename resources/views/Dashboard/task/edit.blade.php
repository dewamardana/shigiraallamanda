...
<div class="bg-white p-5 rounded-xl shadow-2xl m-5 mx-auto max-w-3xl w-full">
  <h2 class="font-bold text-4xl text-center text-black-500 mb-8">✏️ Edit Task "{{ $task->name }}"</h2>
  ...
  <form method="POST" action="{{ route('tasks.update', $task->id) }}">
    ...
    <div class="mb-4">
        ...
        <input type="text" name="name" id="name" value="{{ old('name', $task->name) }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required />
        @error('name')
    ...
    <div class="mb-4">
        ...
        <input type="text" name="point" id="point" value="{{ old('point', $task->point) }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required />
        @error('point')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
        @enderror
    </div>
