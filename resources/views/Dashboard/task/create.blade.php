...
@section('content')
  <div class="bg-white p-5 rounded-xl shadow-2xl m-5 mx-auto max-w-3xl w-full">
    <h2 class="font-bold text-4xl text-center text-black-500 mb-8">
      {{ __('dashboardTask.create.title', ['name' => $taskGroup->name]) }}</h2>
    ...
    <form method="POST" action="{{ route('tasks.store') }}">
      ...
      <div class="flex flex-col gap-5">
        ...
        <div>
          ...
          <input type="number" name="point" id="point" min="0" aria-describedby="helper-text-explanation"
            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 "
            placeholder="{{ __('dashboardTask.form.placeholder_point') }}" required />
          @error('point')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
          @enderror
        </div>
