...
@section('content')
  <div class="bg-white p-5 rounded-xl shadow-2xl m-5 mx-auto max-w-5xl w-full">
    <h2 class="font-bold text-4xl text-center text-black-500 mb-8">{{ $title ?? 'Update Cleaning Group' }}</h2>

    <form action="{{ route('cleaningGroups.update', $group->slug) }}" method="POST" enctype="multipart/form-data">
      @csrf
      @method('PUT')
      ...
      <div class="flex flex-col md:flex-row gap-8 justify-center p-6">
        ...
        <div class="w-full md:max-w-xs">
          <h6 class="mb-2 text-lg font-semibold text-center">Group Image</h6>
          <p class="text-xs text-slate-500 mb-6 text-center">Upload a representative image for the group</p>
          <div
          ...
          <label for="foto" class="block mb-2 text-sm font-medium text-teal-1001">Upload Image</label>
          ...
          <p class="mt-1 text-sm text-teal-1001">Max size 2MB. JPG, PNG only.</p>
        ...
        <div class="w-full space-y-5">
          {{-- Building Name --}}
          <div>
            <label for="building_name" class="block mb-2 text-sm font-medium text-teal-1001">Building Name</label>
            <input type="text" name="building_name" id="building_name"
              value="{{ old('building_name', $group->building_name) }}"
          ...
          {{-- Description --}}
          <div>
            <label for="description" class="block mb-2 text-sm font-medium text-teal-1001">Description</label>
            <textarea name="description" id="description" rows="4"
              class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500">{{ old('description', $group->description) }}</textarea>
            @error('description')
              <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
            @enderror
          </div>
          ...
          <div>
            ...
            <select name="status" id="status"
              ...
              <option value="inactive" {{ old('status', $group->status) == 'inactive' ? 'selected' : '' }}>Inactive
              </option>
            </select>
            @error('status')
              <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
          ...
          {{-- Cleaning Tasks --}}
          <div>
            <label class="block mb-2 text-sm font-medium text-teal-1001">Cleaning Tasks</label>
            <div class="space-y-3">
