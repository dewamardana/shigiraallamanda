...
@section('content')
  <div class="bg-white p-5 rounded-xl shadow-2xl m-5 mx-auto max-w-5xl w-1/2">
    <h2 class="font-bold text-4xl text-center text-black-500 mb-8">{{ $title ?? 'Edit Checker Task' }}</h2>
        ...
        <div class="w-1/2 space-y-5">
          ...
          <div>
            ...
            <select name="type" id="type"
              ...
              focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
              required>
              <option value="boolean" {{ old('type', $checkerTask->type) == 'boolean' ? 'selected' : '' }}>Boolean
              </option>
              <option value="number" {{ old('type', $checkerTask->type) == 'number' ? 'selected' : '' }}>Number</option>
            </select>
            @error('type')
              <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
            @enderror
