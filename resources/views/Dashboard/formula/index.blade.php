...
@elseif (session('error'))
  ...
  <h1 class="text-3xl font-bold mb-8 text-gray-800">{{ __('dashboardFormulaBuilding.index.title') }}</h1>
  ...
  <div class="relative overflow-x-auto shadow-md sm:rounded-lg mt-4">
    <table class="w-full text-sm text-left rtl:text-right text-gray-500">
      <thead class="text-xs text-white bg-teal-1001 uppercase text-center">
        <tr>
          <th scope="col" class="px-6 py-3">{{ __('dashboardFormulaBuilding.index.table.building') }}</th>
          <th scope="col" class="px-6 py-3">{{ __('dashboardFormulaBuilding.index.table.member') }}</th>
          <th scope="col" class="px-6 py-3">{{ __('dashboardFormulaBuilding.index.table.oa') }}</th>
          <th scope="col" class="px-6 py-3">{{ __('dashboardFormulaBuilding.index.table.ov') }}</th>
          <th scope="col" class="px-6 py-3">{{ __('dashboardFormulaBuilding.index.table.stay') }}</th>
          <th scope="col" class="px-6 py-3">{{ __('dashboardFormulaBuilding.index.table.vec') }}</th>
          <th scope="col" class="px-6 py-3">{{ __('dashboardFormulaBuilding.index.table.premier') }}</th>
          <th scope="col" class="px-6 py-3">{{ __('dashboardFormulaBuilding.index.table.action') }}</th>
        </tr>
      </thead>
      <tbody>
        @foreach ($formulas as $f)
          <tr class="bg-white even:bg-teal-50 border-b border-gray-200 hover:bg-yellow-50 text-center">
            ...
            <td class="px-6 py-4">
              <a href="{{ route('formula.edit', ['formula' => $f->id]) }}"
              ...
              <a href="{{ route('formula.show', ['formula' => $f->id]) }}"
              ...
              <form action="{{ route('formula.destroy', ['formula' => $f->id]) }}" method="POST" class="inline-block"
