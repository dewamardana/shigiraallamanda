...
@section('content')
  <div class="bg-white p-5 rounded-xl shadow-2xl m-5 mx-auto max-w-5xl w-full">
    ...
    <div class="flex flex-col items-center mb-8">
      <h2 class="font-bold text-4xl text-center text-black-500 mb-8">{{ __('dashboardFormulaBuilding.show.title') }}</h2>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm text-slate-700 mb-8">
      <div class="bg-slate-100 p-4 rounded-lg">
        <p class="text-xs font-semibold text-slate-500 mb-1">{{ __('dashboardFormulaBuilding.show.member_count') }}</p>
        <p class="text-base font-medium">{{ $formula->member_count }}</p>
      </div>
      <div class="bg-slate-100 p-4 rounded-lg">
        <p class="text-xs font-semibold text-slate-500 mb-1">{{ __('dashboardFormulaBuilding.show.oa') }}</p>
        <p class="text-base font-medium">{{ $formula->oa }}</p>
      </div>
      <div class="bg-slate-100 p-4 rounded-lg">
        <p class="text-xs font-semibold text-slate-500 mb-1">{{ __('dashboardFormulaBuilding.show.ov') }}</p>
        <p class="text-base font-medium">{{ $formula->ov }}</p>
      </div>
      <div class="bg-slate-100 p-4 rounded-lg">
        <p class="text-xs font-semibold text-slate-500 mb-1">{{ __('dashboardFormulaBuilding.show.stay') }}</p>
        <p class="text-base font-medium">{{ $formula->stay }}</p>
      </div>
      <div class="bg-slate-100 p-4 rounded-lg">
        <p class="text-xs font-semibold text-slate-500 mb-1">{{ __('dashboardFormulaBuilding.show.vec') }}</p>
        <p class="text-base font-medium">{{ $formula->vec }}</p>
      </div>
      <div class="bg-slate-100 p-4 rounded-lg">
        <p class="text-xs font-semibold text-slate-500 mb-1">{{ __('dashboardFormulaBuilding.show.premier') }}</p>
