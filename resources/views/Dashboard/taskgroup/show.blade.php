...
<div class="w-full max-w-screen-2xl mx-auto h-[calc(100vh-80px)] overflow-y-auto px-4 py-6">
    ...
    <h1 class="text-3xl font-bold text-gray-800">📋 Tasks for "{{ $taskGroup->name }}"</h1>
        <div class="flex justify-end gap-4">
            <a href="{{ route('tasks.create', ['task_group_id' => $taskGroup->id]) }}" class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-6 py-2.5">
                Tambah Task
            </a>
            ...
            <tbody>
              @foreach($tasks as $task)
                <tr class="bg-white even:bg-teal-50 border-b border-gray-200 hover:bg-yellow-50 text-center">
                    ...
                    <td class="px-6 py-4">
                      <a href="{{ route('tasks.edit', $task->id) }}" class="focus:outline-none text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2 me-2 mb-2">Edit</a>
                      <form action="{{ route('tasks.destroy', $task->id) }}" method="POST" class="inline-block" data-confirm="{{ __('dashboardFormulaIndex.table.delete_confirm') }}">
                          @csrf
                          @method('DELETE')
