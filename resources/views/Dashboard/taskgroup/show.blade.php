...
@elseif (session('danger'))
  ...
  <h1 class="text-3xl font-bold text-gray-800">{{ __('dashboardTask.show.title', ['name' => $taskGroup->name]) }}</h1>
  <div class="flex justify-end gap-4">
    <a href="{{ route('tasks.create', ['task_group_id' => $taskGroup->id]) }}"
  ...
  <div class="relative overflow-x-auto shadow-md sm:rounded-lg mt-4">
    <table class="w-full text-sm text-left rtl:text-right text-gray-500">
      <thead class="text-xs text-white bg-teal-1001 uppercase text-center">
        <tr>
          <th scope="col" class="px-6 py-3">{{ __('dashboardTask.table.task_name') }}</th>
          <th scope="col" class="px-6 py-3">{{ __('dashboardTask.table.point') }}</th>
          <th scope="col" class="px-6 py-3">{{ __('dashboardTask.table.action') }}</th>
        </tr>
      </thead>
      <tbody>
