...
@section('content')
  ...
  <div class="bg-white p-5 rounded-xl shadow m-5 mx-auto max-w-3xl w-full">
    <h2 class="font-bold text-4xl text-center text-black-500 mb-8">{{ __('dashboardTaskGroup.edit.title') }}</h2>
    ...
    <form action="{{ route('task-groups.update', $taskGroup->id) }}" method="POST">
      ...
      <div class="flex flex-col gap-5">
        <div>
          ...
          <input type="text" name="name" id="name" value="{{ old('name', $taskGroup->name) }}"
          ...
          <label for="active" class="inline-flex items-center cursor-pointer">
            ...
            <div class="relative">
              <input type="checkbox" id="active" name="active" value="1" class="sr-only peer"
                {{ old('active', $taskGroup->active) ? 'checked' : '' }}>
