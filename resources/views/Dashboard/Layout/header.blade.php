    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center justify-start rtl:justify-end">
        <button data-drawer-target="logo-sidebar" data-drawer-toggle="logo-sidebar" aria-controls="logo-sidebar"
          type="button"
          class="inline-flex items-center p-2 text-sm text-teal-1000 rounded-lg sm:hidden hover:bg-teal-1000 hover:text-white focus:outline-none focus:ring-2 focus:ring-gray-200">
        ...
        <a href="{{ route('homepage') }}" class="flex ms-2 md:me-24">
          ...
          <span
            class="self-center text-xl text-white font-semibold sm:text-2xl whitespace-nowrap">{{ __('general.brand_name') }}</span>
        </a>
      </div>
      <div class="flex items-center gap-4">
        <div class="flex items-center ms-3">
          <p class="text-white hidden md:block md:text-base font-semibold text-2xl me-4">Welcome,
            {{ Auth::user()->nama }}</p>
          <div>
            <button type="button" class="flex text-sm bg-gray-800 rounded-full focus:ring-4 focus:ring-gray-300"
              ...
              <img class="w-8 h-8 rounded-full"
                src="{{ Auth::user()->foto ? asset('storage/' . Auth::user()->foto) : asset('images/user-profile.png') }}"
                alt="user photo">
            </button>
          </div>
          <div class="z-50 hidden my-4 text-base list-none bg-white divide-y divide-gray-100 rounded-sm shadow-sm"
            ...
            <ul class="py-1" role="none">
              <li>
                <a href="#" class="block px-4 py-2 text-sm text-teal-1001 hover:bg-teal-1001 hover:text-white"
              ...
              <li>
                <a href="#" class="block px-4 py-2 text-sm text-teal-1001 hover:bg-red-400 hover:text-white"
