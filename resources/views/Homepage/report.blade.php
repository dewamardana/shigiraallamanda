...
@section('content')
<div class="mt-4 my-20 mx-4 md:mx-auto md:max-w-3xl p-6 bg-white border border-gray-200 rounded-lg shadow-2xl">
    <!-- FORM Report -->
    ...
    <form action="{{ route('reportStore') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" value="{{ $authUser->nama }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5" readonly/>
        </div>

        <!-- Report Type -->
        <div>
            <label for="report_type" class="block mb-2 text-sm font-medium text-teal-1001">{{ __('report.report_type') }}</label>
            <select id="report_type" name="report_type" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5" required>
        ...
        <div>
            <label for="description" class="block mb-2 text-sm font-medium text-teal-1001">{{ __('report.description') }}</label>
            <textarea name="description" id="description" rows="4" class="block p-2.5 w-full text-sm bg-gray-50 rounded-lg border border-gray-300"></textarea>
            @error('description')
        ...
        <div>
            <div class="flex justify-between items-center mb-3">
                ...
                <button type="button" onclick="addMemberSelect()" class="text-sm px-4 py-2 text-white bg-blue-600 hover:bg-blue-700 rounded-lg shadow-sm">
                    {{ __('report.members.add') }}
                </button>
            </div>
            <div id="memberSelectContainer">
                <div class="flex gap-2 mb-2">
                    <select name="members[]" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5">
                        <option value="">{{ __('report.members.select_member') }}</option>
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}">{{ $user->nama }}</option>
                        @endforeach
                    </select>
                    <button type="button" onclick="removeMemberSelect(this)" class="px-4 py-2 text-white bg-red-600 hover:bg-red-700 rounded-lg shadow-sm">
                        {{ __('button.remove') }}
                    </button>
                </div>
            </div>
