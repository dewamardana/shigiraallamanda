...
<nav class="bg-teal-1001">
  ...
  @auth
    <div class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-language">
      <ul class="flex flex-col font-medium p-4 md:p-0 mt-4 rounded-lg bg-teal-1001 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-teal-1001">
        <li>
          <a href="{{ route('homepage') }}" class="block py-2 px-3 text-gold rounded-sm hover:bg-teal-1000 md:hover:text-white">{{ __('HomepageLayout.header.menu.homepage') }}</a>
        </li>
        @can('cleaning')
          <li>
            <a href="{{ route('cleaning') }}" class="block py-2 px-3 text-gold rounded-sm hover:bg-teal-1000 md:hover:text-white ">{{ __('HomepageLayout.header.menu.cleaning') }}</a>
          </li>
        @endcan
        @can('checker')
          <li>
            <a href="{{ route('checker') }}" class="block py-2 px-3 text-gold rounded-sm hover:bg-teal-1000 md:hover:text-white ">{{ __('HomepageLayout.header.menu.checker') }}</a>
          </li>    
        @endcan
        @can('office')
          <li>
            <a href="{{ route('office') }}" class="block py-2 px-3 text-gold rounded-sm hover:bg-teal-1000 md:hover:text-white ">{{ __('HomepageLayout.header.menu.office') }}</a>
          </li>
        @endcan
        @can('admin')
          <li>
            <a href="{{ route('dashboard') }}" class="block py-2 px-3 text-gold rounded-sm hover:bg-teal-1000 md:hover:text-white ">{{ __('HomepageLayout.header.menu.dashboard') }}</a>
          </li>
        @endcan
      </ul>
  </div>
