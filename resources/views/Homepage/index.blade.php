...
<div class="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6 p-4 items-stretch mb-36 md:mb-0">
    ...
    <div class="lg:col-span-2 h-full">
        <div class="w-full bg-white rounded-lg shadow-2xl">
            <div class="flex justify-end px-4 pt-4">
                <button id="dropdownButton" data-dropdown-toggle="dropdown" class="inline-block text-gray-500 hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg text-sm p-1.5" type="button">
                    ...
                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 16 3">
                        <path d="M2 0a1.5 1.5 0 1 1 0 3A1.5 1.5 0 0 1 2 0Zm6.041 0a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM14 0a1.5 1.5 0 1 1 0 3A1.5 1.5 0 0 1 14 0Z"/>
                    </svg>
                </button>
            ...
            <div class="flex flex-col items-center pb-12 px-4">
                <img class="w-36 h-36 mb-3 rounded-full shadow-lg" src="{{ $user->foto ? asset('storage/' . $user->foto) : asset('images/user-profile.png') }}" alt="User Image"/>
                <h5 class="mb-1 text-xl font-medium text-teal-800">{{ $user->nama }}</h5>
                <span class="text-md text-black">{{ $user->department }}</span>
                <div class="grid grid-cols-2 gap-3 mt-4">
                    @can('cleaning')
                        <a href="{{ route('cleaning') }}"><button type="button" class="text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-4 focus:ring-blue-300 font-medium rounded-sm text-sm w-full px-5 py-2.5 text-center me-2 mb-2">{{ __('homepageIndex.cleaning') }}</button></a>
                    @endcan
                    @can('checker')
                        <a href="{{ route('checker') }}"><button type="button" class="text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-4 focus:ring-green-300 font-medium rounded-sm text-sm w-full px-5 py-2.5 text-center me-2 mb-2">{{ __('homepageIndex.checker') }}</button></a>
                    @endcan
                    @can('office')
                        <a href="{{ route('office') }}"><button type="button" class="text-white bg-yellow-600 hover:bg-yellow-700 focus:outline-none focus:ring-4 focus:ring-yellow-300 font-medium rounded-sm text-sm w-full px-5 py-2.5 text-center me-2 mb-2">{{ __('homepageIndex.office') }}</button></a>
                    @endcan
                    <a href="{{ route('history') }}"><button type="button" class="text-white bg-purple-600 hover:bg-purple-700 focus:outline-none focus:ring-4 focus:ring-purple-300 font-medium rounded-sm text-sm w-full px-5 py-2.5 text-center me-2 mb-2 ">{{ __('homepageIndex.history') }}</button></a>
                    <a href="{{ route('report') }}"><button type="button" class="text-white bg-yellow-400 hover:bg-yellow-500 focus:outline-none focus:ring-4 focus:ring-yellow-300 font-medium rounded-sm text-sm w-full px-5 py-2.5 text-center me-2 mb-2 ">{{ __('homepageIndex.report') }}</button></a>            
    ...
    <div class="bg-white rounded-lg shadow-2xl p-6 space-y-6 h-full">
        ...
        <div class="flex items-center gap-3">
            <img src="https://randomuser.me/api/portraits/women/65.jpg" class="w-12 h-12 rounded-full" alt="">
            <div>
