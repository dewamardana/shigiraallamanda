  ...
  <div
    ...
    <form action="{{ route('cleaningStore') }}" method="POST">
      ...
      <div class="mb-6">
        ...
        <select id="building" name="cleaning_group_id"
          ...
          @foreach ($building as $build)
            <option value="{{ $build->id }}" {{ old('cleaning_group_id') == $build->id ? 'selected' : '' }}>
      ...
      <div>
        ...
        <div id="taskContainer" class="grid grid-cols-2 md:grid-cols-4 gap-4">
          {{-- diisi dari JS --}}
        </div>
      </div>
...
@section('script')
  <script>
    ...
    function updateTotal() {
      const taskInputs = document.querySelectorAll('#taskContainer input[type="text"]');
      let total = 0;
              ...
              div.innerHTML = `
                ...
                <div class="relative flex items-center rounded-lg shadow-sm overflow-hidden">
                    <button type="button" onclick="decrement('task_${task.id}')" 
                        class="bg-gray-100 hover:bg-gray-200 border border-gray-300 p-2 h-11 rounded-l-lg">
                        <i data-feather="minus" class="text-accent-1000"></i>
                    </button>

                    <input type="text" name="tasks[${task.id}]" id="task_${task.id}" value="0" min="0"
                        class="w-full text-center text-sm font-medium bg-gray-50 h-11 border-t border-b border-gray-300 outline-none" />

                    <button type="button" onclick="increment('task_${task.id}')" 
                        class="bg-gray-100 hover:bg-gray-200 border border-gray-300 p-2 h-11 rounded-r-lg">
